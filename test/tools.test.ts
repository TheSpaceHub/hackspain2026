...
check('bare STT clock becomes an exact clock', resolveWhen('at 2 30', constraintNow).at_clock, { hour: 2, minute: 30, ambiguous: true });
check('half past two parses as thirty minutes', resolveWhen('half past two', constraintNow).at_clock, { hour: 2, minute: 30, ambiguous: true });
check('bare at clock is parsed', resolveWhen('at 9', constraintNow).at_clock, { hour: 9, minute: 0, ambiguous: true });
