...
export type View = 'overview' | 'live' | 'finished';
...
/** `#/overview`, `#/live/<id>`, `#/finished/<id>`. The older `#/calls/<id>` still opens a finished call. */
function parse(hash: string): Route {
  const m = /^#\/(overview|live|finished|calls)(?:\/(.+))?$/.exec(hash);
  if (!m) return DEFAULT;
